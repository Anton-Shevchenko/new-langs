...
func (h *AppRouter) OnTestAnswer(ctx context.Context, b *bot.Bot, mes models.MaybeInaccessibleMessage, answerDataBytes []byte) {
...
func (h *AppRouter) OnWriteTestAnswer(ctx context.Context, b *bot.Bot, update *models.Update, user *model.User) {
...
func (h *AppRouter) OnWriteTestOption(ctx context.Context, b *bot.Bot, mes models.MaybeInaccessibleMessage, data []byte) {
...
func (h *AppRouter) sendTestResultMessage(ctx context.Context, chatID int64, messageID int, word *model.Word, user *model.User, correct bool) {
	statusEmoji := "✅"
	if !correct {
		statusEmoji = "❌"
	}
...
func updateWordRate(h *AppRouter, word *model.Word, correct bool) {
	if correct {
		word.Rate++
	} else {
		word.Rate--
...
func isCorrect(input, target string) bool {
	...
	return strings.EqualFold(inputWord, targetWord)
}
